...
// Динамический импорт Firebase, если window.db не определён
async function initializeFirebase() {
    if (!window.db) {
        try {
            const { initializeApp, getDatabase } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js');
            ...
            window.firebaseFunctions = { ref, get, update }; // Экспортируем функции для использования
            console.log("Firebase инициализирован динамически");
        } catch (error) {
...
waitForTelegram().then(() => {
    ...
    initializeFirebase().then(() => {
        ...
        function saveScore() {
            ...
            if (!window.db || !window.firebaseFunctions) {
                console.error("Firebase не инициализирован! Проверьте подключение.");
                return;
            }
    ...
    }).catch(error => console.error("Ошибка при инициализации Firebase:", error));
